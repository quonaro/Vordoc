...
func (h *DocsHandler) ServeLogo(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := os.Stat(logoPath); err != nil { // #nosec G703 — path is validated by contentProvider
	...
	http.ServeFile(w, r, logoPath) // #nosec G703 — path is validated by contentProvider
}
...
func (h *DocsHandler) ServeAsset(w http.ResponseWriter, r *http.Request) {
	...
	http.ServeFile(w, r, filePath) // #nosec G703 — path is validated by contentProvider
}
...
func serveSVGWithTitle(w http.ResponseWriter, r *http.Request, filePath, fileName string) error {
	data, err := os.ReadFile(filePath) // #nosec G304 G703 — path is validated by contentProvider
	if err != nil {
		return err
	}
	...
	fi, err := os.Stat(filePath) // #nosec G703 — path is validated by contentProvider
	if err != nil {
		return err
	}

	http.ServeContent(w, r, filePath, fi.ModTime(), bytes.NewReader(data)) // #nosec G703 — path is validated by contentProvider
	return nil
}
